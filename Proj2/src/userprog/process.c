...
process_execute (const char *file_name) 
{
  ...
  // Proj 1 implement : process_execute
  char temp_file_name[100];
  char *temp;
	strlcpy(temp_file_name, file_name, strlen(file_name)+1);
	char *file_name2 = strtok_r(temp_file_name, " ", &temp);
  char *temp_file_name2 = file_name2;

  struct file *file = NULL;
	file = filesys_open(temp_file_name2);
  ...
  // Proj 1 implement
  struct thread *child = find_thread_with_tid(tid);
  if(child != NULL) {
    list_push_back(&thread_current()->child, &child->childelem);
...
start_process (void *file_name_)
{
  ...
  // Proj 2 implement
  sema_up(&thread_current()->parent->file_sema);
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  // Proj 1 implement : parse file name
  int argc = 0;
  char *argv[100];
  char *next_str, *temp_file_name, *temp_str;
  char str[100];

  strlcpy(str, file_name, strlen(file_name)+1);
  temp_file_name = strtok_r(str, " ", &next_str);
  temp_str = temp_file_name;
  while(temp_str != NULL){
    argv[argc++] = temp_str;
    temp_str = strtok_r(NULL, " ", &next_str);
  }
  ...
  // Proj 1 implement : Argument Passing
  int len, total_len = 0;
  char *temp_argv[100];

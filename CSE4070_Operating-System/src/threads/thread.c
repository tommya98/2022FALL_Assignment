...
thread_set_nice (int new_nice) 
{
  ...
  old_level = intr_disable();
  t->nice = new_nice;
  t->priority = float_to_int(float_sub_float(float_sub_float(float_add_int(0, PRI_MAX), float_div_int(t->recent_cpu, 4)), float_mul_int(float_add_int(0, t->nice), 2)));
...
void thread_sleep(int64_t ticks) {
  ...
  cur->sleeptime = ticks;
  list_push_back(&blocked_queue, &cur->elem);
  thread_block();
  intr_set_level(old_level);
}
